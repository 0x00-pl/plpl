...
def read_object(it):    
    ...
    while True:
        ...
        read_space(it)
        cur_data1= read_value(it)
        
        read_space(it)
        c=next(it)
        if c==':':
            cur_data1= str(cur_data1)
            cur_data2= read_value(it)
            name[cur_data1]= len(data)
        ...
        if c=='}':
            ...
            continue
        if c!=',':
            handle_err(str({'line':line,'col':col})+\
                       ':: not a object. missing "," or "}" ?')
        ...
        for ln in fs:
            for c in ln:
                while (yield c): # .send(True) to read last char
                    yield None # yield to fix .send(True)
                char+=1
                col+=1
